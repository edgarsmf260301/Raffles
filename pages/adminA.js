...
const AdminA = () => {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      ...
      try {
        const response = await axios.get('/api/customersAdmin?status=Aprobado', {
  ...
  const handleStatusChange = async () => {
    ...
    try {
      const response = await axios.put(`/api/customersAdmin?id=${selectedCustomer._id}`, {
        status,
        fromAdminA: true // Indicate that the request is from adminA
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        <div className={styles.header}>
          <h2 className={styles.title}>Aprobados</h2>
          <LogoutButton />
        </div>
